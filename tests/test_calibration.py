...
class CalibrationTests(unittest.TestCase):
    ...
    def test_small_sample_uses_worst_observed_overprediction(self) -> None:
        profile = build_profile(
            [
    ...
    def test_ten_samples_use_tenth_percentile_not_single_outlier(self) -> None:
        ratios = [-0.9, -0.2, -0.18, -0.15, -0.1, -0.08, -0.05, 0.0, 0.02, 0.05]
        daylight = [
            {"error_kwh": ratio * 10.0, "error_ratio": ratio}
            for ratio in ratios
        ]
        overnight = [{"drop_rate_kw": 1.0 + i * 0.1} for i in range(10)]
