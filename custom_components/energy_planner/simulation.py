...
class ControllerSettings:
    """Forecast mirror of the fast EcoFlow surplus-controller policy."""
...
def full_day_solar_curve(
    *,
    ...
    """Return a smooth, energy-conserving, peak-time-aware solar curve in kW.

    Two half-sine lobes meet at the supplied peak. Their combined area is
    ``2 / pi * peak_kw * daylight_hours``, so the total forecast energy is
    conserved regardless of where the peak occurs.
    """
...
def simulate_energy_flow(
    *,
    ...
    """Simulate AC energy flow with the fast controller in quasi-steady state.

    The controller is iterated to a stable command inside each forecast step.
    That intentionally models the controller's fast response without pretending
    a day-ahead forecast can predict second-by-second cloud transients.
    """
    ...
    socs = [
        _clamp(float(value), 0.0, 100.0)
        for value in bank_socs_pct
    ]
    ...
    while elapsed_h < duration_h - 1e-9:
        ...
        # Iterate a handful of fast controller reactions at the same forecast
        # point. In the real integration, grid changes retrigger after ~2 s;
        # forecast steps are much coarser, so converging here is more faithful
        # than applying only one controller reaction per minute.
        for _ in range(12):
        ...
        active_bits = tuple(bit for bit in (1, 2, 4) if controller_state.mask & bit)
        commanded_rate_w = float(controller_state.rate_w)
        accepted_ac_by_bank = [0.0, 0.0, 0.0]

        for index, bit in enumerate((1, 2, 4)):
            if bit not in active_bits:
                continue
            commanded_ac_kwh = commanded_rate_w / 1000.0 * dt_h
            if ignore_battery_capacity:
        ...
        has_headroom = ignore_battery_capacity or any(eligible)
        if export_w > 0:
            ...
            if not has_headroom:
                ...
                controller_power_ceiling_w = controller.maximum_rate_w * sum(eligible)
                power_piece_w = min(
                    remaining_export_w,
            ...
            control_piece_w = max(
                export_w - power_piece_w - capacity_piece_w,
                0.0,
            )
    ...
    aggregate_soc = (
        100.0 * sum(stored) / total_capacity
        if total_capacity > 0
        else 0.0
    )
